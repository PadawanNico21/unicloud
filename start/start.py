...
def add_user():
    ...
    ShellCmd(
        f"useradd -u {user_uid} -g {user_uid} -s /bin/bash -c 'unison sync user' -d /data {user}"
    )
...
def gen_key():
    ...
    if role == "client" or role == "replica_server":
        cmd = ShellCmd(
            f"su -c \"ssh-keygen -f /data/.ssh/id_rsa -t rsa -N '' \" {user}"
        )
        print(cmd)
...
def start_supervisord():
    ...
    ShellCmd(
        f"/usr/bin/supervisord --configuration {supervise_cfg} --logfile {supervise_log}"
    )
...
def server_conf():
    ...
    ShellCmd(
        "sed -i 's/\\/var\\/log\\/nginx\\/access.log/\\/data\\/log\\/access.log/g' /etc/nginx/nginx.conf"
    )
...
def exit_screen(status, error="None", role="None"):
    ...
    if status == "error_wrong_permission":
        print(f"-=(: [{role}] Unicloud Permission ERROR")
        print(
            f"User {user} with uid {user_uid} can't write to mounted volume {root_dir}!"
        )
        print("Do not run docker/docker-compose as root!!")
        print(f"Make sure {user_uid} is a valid id on your system")
        print("On Linux run 'id' command to know your userid")
        print("Change USER_UID env var according.")
        print("Exit container now..")
        sys.exit(500)
        return
